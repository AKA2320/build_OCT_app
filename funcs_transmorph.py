...
class RegistrationHead(nn.Module):
    def __init__(self, in_channels):
        super().__init__()
        # Global average pooling to reduce spatial dims to 1×1
        self.global_pool = nn.AdaptiveAvgPool2d((1, 1))
        
        # Linear projection to 2 values: dx and dy
        self.fc = nn.Linear(in_channels, 2)
        
        # Initialize weights like original code
        nn.init.normal_(self.fc.weight, mean=0, std=1e-5)
        nn.init.constant_(self.fc.bias, 0.0)

    def forward(self, x):
        """
        x: (B, C, H, W)
        returns: (B, 2) translation vector [dx, dy] per batch element
        """
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, src, translation):
        # new locations
        B, C, H, W = src.shape
        # If translation is in pixels, normalize: t_norm = t / [(W-1)/2, (H-1)/2]
        t_norm = translation.clone()
        t_norm[:, 0] /= (W - 1) / 2
        t_norm[:, 1] /= (H - 1) / 2

        # Build affine matrix
        theta = torch.eye(3, device=src.device).unsqueeze(0).repeat(B, 1, 1)
        theta[:, 0, 2] = t_norm[:, 0].double()
        theta[:, 1, 2] = t_norm[:, 1].double()
        # print(src.size(),self.size)
        # grid_sample expects a 2×3 matrix per batch
        grid = F.affine_grid(theta[:, :2, :], size=src.size(), align_corners=True)
        warped = F.grid_sample(src.double(), grid.double(), align_corners=True, mode = self.mode)
        return warped
...
class TransMorph(nn.Module):
    def __init__(self, config):
        ...
        #     kernel_size=3,
        # )
        self.reg_head = RegistrationHead(in_channels=config.reg_head_chan)
    ...
    def forward(self, x):
        ...
        tran_flow = self.reg_head(x)
        out = self.spatial_trans(source, tran_flow)
...
class NCCLoss(nn.Module):
    ...
    def forward(self, I, J):
        """
        I, J: (B, 1, H, W) or (B, C, H, W) — fixed and moved images
        Returns: mean local normalized cross-correlation (scalar)
        """
        ndims = len(I.shape) - 2
        assert ndims == 2, "Only 2D NCC supported in this version"

        padding = self.win_size // 2
        sum_filt = torch.ones((1, I.shape[1], self.win_size, self.win_size), device=I.device).double()

        I2 = I * I
        J2 = J * J
        IJ = I * J

        I_sum = F.conv2d(I,     sum_filt, padding=padding)
        J_sum = F.conv2d(J,     sum_filt, padding=padding)
